...
//import edu.washington.distantsupervision.annotation.MultiRInstance;
//import edu.washington.distantsupervision.annotation.MultiRReader;
import edu.washington.distantsupervision.util.NoDependencyPathException;


public class InstanceFeatureGenerator implements FeatureGenerator {
    ...
    private static InstanceFeatureGenerator instance = null;
	
	public static InstanceFeatureGenerator getInstance(){
		if(instance == null) instance = new InstanceFeatureGenerator();
		return instance;
		}	
	
	public InstanceFeatureGenerator() {
		lex = "lex#";
		dep0 = "path0#";	// only middle
		dep1 = "path1#";	// left + middle
		dep2 = "path2#";	// middle + right
		dep3 = "path3#";	// left + middle + right
	}
	...
	public static void main(String[] args) throws Exception {
		 ...
		 */
		/*InstanceFeatureGenerator feagen = new InstanceFeatureGenerator();
		File outputDir = new File(args[1]);
		BufferedWriter featureFile = new BufferedWriter(
				new OutputStreamWriter(
						new FileOutputStream(
								new File(outputDir, "training.usfeat")), "UTF-8"));
		...
		for (MultiRInstance trainInst : trainInsts) {
			...
			System.out.println(trainInst.getSentenceID());
	...
	public List<String> generateFeatures(String[] tokens, String[] pos, 
        ...
        if (ner.length != NumOfTokens) {
        	...
        	// System.out.println(entity1 + " " + e1StartOffset + " " + e1EndOffset 
        	// 		+ entity2 + " " + e2StartOffset + " " + e2EndOffset + " "
        	// 		+ relation);
        	System.exit(1);
        }
		...
		//-----------------------------------------------------
        List<String> featureList = USchemaFeatures(tokens,
	 ...
	 * @return
	 */
	public List<String> USchemaFeatures( String[] tokens, 
			String[] postags,
		...
		features.add(lex + inv + "|" + firstNer + "|" + mto + "|" + secondNer);
		features.add(lex + inv + "|" + prefixes[1] + "|" + firstNer + "|" + mto + "|" + secondNer + "|" + suffixes[1]);
		features.add(lex + inv + "|" + prefixes[2] + "|" + firstNer + "|" + mto + "|" + secondNer + "|" + suffixes[2]);
		...
		if (lca < 0) {
			...
			features.add(dep0 + "no_dependency_path");
			return features;
		...
		if (arg1strs.size() > 0 && arg2strs.size() > 0) {
			for (String w1 : arg1strs)
				for (String w2 : arg2strs)
					features.add(dep3 + "str:" + w1 + " |" + basicStr + "| " + w2);
		}
		...
		for (int i=0; i < arg1dirs.size(); i++) {
			features.add(dep1 + "str:" + arg1strs.get(i) + " |" + basicStr);
		...
		for (int i=0; i < arg2dirs.size(); i++) {
			features.add(dep2 + "str:" + basicStr + "| " + arg2strs.get(i));
			features.add("dep:" + basicDep + "| " + arg2deps.get(i));
			features.add("dir:" + basicDir + "| " + arg2dirs.get(i));
		}

		features.add(dep0 + "str:" + basicStr);

		return features;
	}

	//This is called by:
	//DocumentExtractor.extractFromSententialInstance
